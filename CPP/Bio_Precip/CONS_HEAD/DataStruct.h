...
extern double D_cahco3;  //the diffusion coefficient for [CaHCO3+]
extern double D_caco3;  //the diffusion coefficient for aqueous [CaCO3]
...
extern double K_cahco3;  // Equilibrium constant for [CaHCO3-] <-> [Ca2+] + [HCO3-], K4 in the function
extern double K_caco3;  // Equilibrium constant for [CaCO3] <-> [Ca2+] + [CO3^2-], K5 in the function
extern double K_so;      //equilibrium calcite solubility product
extern double A_Davies, B_Davies, alpha_Davies;  //constants used in Davies equation for activity coefficient
...
extern const double Z_ur, Z_nh3, Z_nh4, Z_h2co3, Z_hco3, Z_co3, Z_ca, Z_cl, Z_h, Z_oh, Z_c, Z_electron, Z_cahco3, Z_caco3;
extern double a_EPU;     //the unit constant for electric potential
extern double L_theta;   //The ratio of computation domain length to the whole domain length (< 1
extern double Q_flux;    //the flux from the left boundary, induced by given pressure drop
extern double U_ref;     //the maximum velocity at the centerline of the left boundary induced by given pressure drop
extern double G_pres;    //the linear pressure gradient induced by the 
extern double Re_s;      //Reynolds of the solvent
extern int Newton_max_k; // maximum iteratoin number for Newton's method
extern double Newton_tol; //Tolerance for Newton's method
extern int Activity_max_k; //maximum iteration number for activity coefficients update
extern double Activity_tol; //Tolerance for successive update of activity coefficients
...
public:
    ...
    double dx, dy, T_final, CFL, GMRES_tol, rho_0, c_0, h, t_0, LL, L, tfac, alph, eps_dif, Shear_v, Delta_P; 

    double Ini_phc, Ini_phb, Ini_Ur, Ini_NH3, Ini_NH4, Ini_H2CO3, Ini_HCO3, Ini_CO3, Ini_Ca, Ini_Cl, Ini_H, Ini_OH, Ini_CaHCO3, Ini_CaCO3,
                    Ini_c, cutoff_c_time, D_phic_tol, D_delta, a_epu;

    size_t Nx, k;

    int Init_flag,  BC_flag, Grid_flag,  velo_flag, Init_N,  GMRES_max_k, F_Stride, N_dif_steps, D_flag, Difu_Init_flag, Ave_Vis_flag, EP_flag, Ion_Act_flag;
...
public:
    ...
    double gam_0, gam_1, gam_2, KB, T, kai_CH, lam_c, lam_b, lam_s, s_eps, s_mu, s_K_c, s_A, ds, d_ur, d_nh3, d_nh4, d_h2co3, d_hco3, d_co3, d_ca, d_cl, d_h, d_oh, d_cahco3, d_caco3
           eta_b, eta_s, eta_c, rho_b, rho_s, rho_c, kur, kp, s_crit;
           
    double np_inv, phi_reg, phis_reg, kacid_1, kacid_2, k_nh, k_w, k_cahco3, k_caco3, k_so, ca_phase, ca_vol_coef, vol_penal;
