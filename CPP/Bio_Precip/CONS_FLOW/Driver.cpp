...
void Time_Evolve(char* filename){
  ...
  valarray<double> Out_u(0.0, Ny + 1);
  valarray<double> Out_v(0.0, Ny + 1);  
...
//   valarray<double> Rc(0.0, Ny+1);
  ...
  if(Init_flag >= 0)
      ...
      if(CP.D_flag == 1)
	{
      ...
      Write_All_Node_Data(SaveDir, Inter_file_NO, un, vn, phic_n, phib_n, Ur_n, NH3_n, NH4_n, H2CO3_n, HCO3_n, CO3_n, Ca_n, Cl_n, H_n, OH_n, NT, CT, EP, SS_n, c_n, 
                          T_current, CP);
  ...
  while(T_current < T_final) 
    {
      ...
      Cal_Fast_Species_Node(H_n, NT, K_nh, K_w, CT, K_acid_1, K_acid_2, Ca_n, Cl_n, CHA_SUM, NH3_n, NH4_n, H2CO3_n, HCO3_n, CO3_n);
...
// #endif
	...
	time(&lin_s);
	GMRES(A_EP, F_EP, d_EP, 5e-4, GMRES_max_k, GMRES_flag, GMRES_res, GMRES_num_restart, GMRES_num_k);
	time(&lin_e);
	...
	Update_EP(EP, d_EP);
      ...
      //calculate the saturation state of CaCO3, this is done after the fast reactions
      SS_n = Saturation_State(Ca_n, CO3_n);
      ...
      Cal_Drift_U(Drift_U, T_current + dt_n, dy, Ny, CP);
...
// #endif
      ...
      //calculate the out-flow velocity by the drift BC
      Cal_Drift_R_BC(Out_u, Drift_U, un[Nx+1], un[Nx], dx, dy, dt_n, Ny, 0.0);
      Cal_Drift_R_BC(Out_v, Drift_U, vn[Nx+1], vn[Nx], dx, dy, dt_n, Ny, 0.0);

      //update velocity
      //buid the momentum equation according to BC_flag
     if(Grid_flag == 0) //regular grid
       Build_NSE_RHS_REG_GRID(Fx, Fy, rho_n, mu, Drift_U, Out_u, Out_v, un, vn, s, phib_n, phic_n, dx, dy, dt_n, CP);  
     else if(Grid_flag == 1)  //half-staggered grid
       Build_NSE_RHS_STG_GRID(Fx, Fy, rho_n, mu, Drift_U, Out_u, Out_v, un, vn, s_stg, phib_n, phic_n, dx, dy, dt_n, CP);
      ...
      if((n-1)%F_Stride == 0) //out result every F_Stride steps
	{
      ...
      Cal_Drift_R_BC(Rph_np1, Drift_U, phic_n[Nx+1], phic_n[Nx], dx, dy, dt_n, Ny, 0.0);

      //calculate Laplacian phi at t_n+1 at i = Nx + 1
      Cal_Drift_R_BC(Lap_phc_np1_Nxp1, Drift_U, Lap_phc_n_Nxp1, Lap_phc_n_Nx, dx, dy, dt_n, Ny, 0.0);
...
//       //*************************************DEBUG*********************************************
      ...
      if((n-1)%F_Stride == 0) //out result every F_Stride steps
	{
      ...
      //calculate phi at t_n+1, i = Nx + 1
      Cal_Drift_R_BC(Rph_np1, Drift_U, phib_n[Nx+1], phib_n[Nx], dx, dy, dt_n, Ny, 0.0);

      //calculate Laplacian phi at t_n+1 at i = Nx + 1
      Cal_Drift_R_BC(Lap_phb_np1_Nxp1, Drift_U, Lap_phb_n_Nxp1, Lap_phb_n_Nx, dx, dy, dt_n, Ny, 0.0);
...
//       Cal_Lc(Lc, T_current, dy, Ny, CP.Ini_Ur);
      ...
      Cal_Drift_R_BC(Rc_new[1], Drift_U, Rc_old[1], Ur_n[Nx+1], dx, dy, dt_n, Ny, D_ur);
...
//       Cal_Lc(Lc, T_current, dy, Ny, CP.Ini_NH3);
      ...
      Cal_Drift_R_BC(Rc_new[2], Drift_U, Rc_old[2], NH3_n[Nx+1], dx, dy, dt_n, Ny, D_nh3);
...
//       Cal_Lc(Lc, T_current, dy, Ny, CP.Ini_NH4);
      ...
      Cal_Drift_R_BC(Rc_new[4], Drift_U, Rc_old[4], NH4_n[Nx+1], dx, dy, dt_n, Ny, D_nh4);
...
//       Cal_Lc(Lc, T_current, dy, Ny, CP.Ini_H2CO3);
      ...
      Cal_Drift_R_BC(Rc_new[3], Drift_U, Rc_old[3], H2CO3_n[Nx+1], dx, dy, dt_n, Ny, D_h2co3);
...
//       Cal_Lc(Lc, T_current, dy, Ny, CP.Ini_HCO3);
      ...
      Cal_Drift_R_BC(Rc_new[6], Drift_U, Rc_old[6], HCO3_n[Nx+1], dx, dy, dt_n, Ny, D_hco3);
...
//       Cal_Lc(Lc, T_current, dy, Ny, CP.Ini_CO3);
      ...
      Cal_Drift_R_BC(Rc_new[7], Drift_U, Rc_old[7], CO3_n[Nx+1], dx, dy, dt_n, Ny, D_co3);
...
//       Cal_Lc(Lc, T_current, dy, Ny, CP.Ini_Ca);
      ...
      Cal_Drift_R_BC(Rc_new[8], Drift_U, Rc_old[8], Ca_n[Nx+1], dx, dy, dt_n, Ny, D_ca);
...
// 	  sprintf(debfile,"%sRc_Ca_%d",SaveDir,Inter_file_NO);
// 	  Write_Vector(debfile, Rc, Ny+1);
// 	  sprintf(debfile,"%sLc_Ca_%d",SaveDir,Inter_file_NO);
...
//       Cal_Lc(Lc, T_current, dy, Ny, CP.Ini_Cl);
      ...
      Cal_Drift_R_BC(Rc_new[9], Drift_U, Rc_old[9], Cl_n[Nx+1], dx, dy, dt_n, Ny, D_cl);
...
//       Cal_Lc(Lc, T_current, dy, Ny, CP.Ini_H);
      ...
      Cal_Drift_R_BC(Rc_new[10], Drift_U, Rc_old[10], H_n[Nx+1], dx, dy, dt_n, Ny, D_h);
...
//       Cal_Lc(Lc, T_current, dy, Ny, CP.Ini_OH);
      ...
      Cal_Drift_R_BC(Rc_new[5], Drift_U, Rc_old[5], OH_n[Nx+1], dx, dy, dt_n, Ny, D_oh);
...
// 	  Cal_Lc(Lc, T_current, dy, Ny, CP.Ini_c);
	  ...
	  Cal_Drift_R_BC(Rc_new[11], Drift_U, Rc_old[11], c_n[Nx+1], dx, dy, dt_n, Ny, Ds);
      ...
      if((n-1)%F_Stride == 0) //out result every F_Stride steps
	{
	  Write_All_Node_Data(SaveDir, Inter_file_NO, un, vn, phic_n, phib_n, Ur_n, NH3_n, NH4_n, H2CO3_n, HCO3_n, CO3_n, Ca_n, Cl_n, H_n, OH_n,
                              NT, CT, EP, SS_n, c_n, T_current, CP);
...
//             Write_Array(debfile, s_stg, (Nx+1)*Ny, Nx+1);
	  ...
	  of_Effluent << Inter_file_NO << " ";
          ...
          for(int Ej = 0; Ej <= 11; Ej++)
	    of_Effluent << Efflu_curr[Ej] << " ";
  
          of_Effluent << Efflu_curr[12] << endl;
