...
%         f1(j) =  .5*dt*u(j+1)/dx - 0.5*(dt/dx)*(x(j) + .5*dx)*(L_dot/L); % the superdiagonal
        ...
        d(j) = 1 + flag_kg_WR*Kg_WR*dt + (.5*dt/dx)*(u(j+1) + u(j)) + (dt/dx)*(x(j) + .5*dx)*(L_dot/L);  % the main diagonal
        
        e1(j) =  - .5*dt*(u(j)+u(j-1))/dx; %  % the subdiagonal 
        
        f1(j) =  - (dt/dx)*(x(j) + .5*dx)*(L_dot/L); % the superdiagonal   Upwind for L_dot/L term
